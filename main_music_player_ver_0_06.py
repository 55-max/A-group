...
# 初期化
# もし、~/musicがあるなら、なにもしない、ないなら、作る。
shutil.os.makedirs('~/music', exist_ok=True)
...
class music_player:
    ...
    def play_music(self):
        ...
        # pathlibを使って、文字化け防止しつつ、ファイルパスを取得する。
        music_path = pathlib.Path('./A-group/data/music/test_music.mp3')

        # music_path = 
        # 一旦、~/music/に置いてみる。
        shutil.copyfile(music_path, '/home/pi/music/test_music.mp3')
        new_music_path = '/home/pi/music/test_music.mp3'

        # os.chdir('/home/pi/music')
        self.player.set_mrl(new_music_path)
        self.player.play()
